...
static etError SHT3X_StartWriteAccess(void)
{
  ...
  i2c2_write_byte(_i2cAddress << 1);
	if(i2c2_waitack()) error = ACK_ERROR;
...
static etError SHT3X_StartReadAccess(void)
{
  ...
  i2c2_write_byte(_i2cAddress << 1 | 0x01);
	if(i2c2_waitack()) error = ACK_ERROR;
...
static etError SHT3X_WriteCommand(etCommands command)
{
  ...
  i2c2_write_byte(command >> 8);
	if(i2c2_waitack()) error = ACK_ERROR;

  // write the lower 8 bits of the command to the sensor
  i2c2_write_byte(command & 0xFF);
	if(i2c2_waitack()) error |= ACK_ERROR;
                                      ...
                                      u8t timeout)
{
  ...
  bytes[0] = i2c2_read_byte();
	i2c2_ack();
	
  if(error == NO_ERROR) 
	{
		bytes[1] = i2c2_read_byte();
		i2c2_ack();
	}
	
  if(error == NO_ERROR) 
	{
		checksum = i2c2_read_byte();
		if(finaleAckNack==NACK) i2c2_noack();
		else i2c2_ack();
	}
...
static etError SHT3X_Write2BytesAndCrc(u16t data)
{
  ...
  i2c2_write_byte(bytes[0]); // write data MSB
	if(i2c2_waitack()) error = ACK_ERROR;
	
  if(error == NO_ERROR) 
	{
		i2c2_write_byte(bytes[1]); // write data LSB
		if(i2c2_waitack()) error = ACK_ERROR;
	}
	
  if(error == NO_ERROR) 
	{
		i2c2_write_byte(checksum); // write checksum
		if(i2c2_waitack()) error = ACK_ERROR;
	}
